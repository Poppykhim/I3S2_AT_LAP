...
public class JsonPlaceholderApiTest {
    ...
    public Object[][] getUserCreationData() {
        return new Object[][]{
                ...
                {"Bob Jones", "bobjones", "bob@example.com"},
                {"Charlie Brown", "charliebrown", "charlie@example.com"}
        };
    }

    @Test(dataProvider = "userCreationData", priority = 1)
    public void testCreateUser_DataDriven(String name, String username, String email) {
        ...
        Assert.assertEquals(response.jsonPath().getString("username"), username, "Username mismatch");
        Assert.assertEquals(response.jsonPath().getString("email"), email, "Email mismatch");
    ...
    public void testGetUser_SchemaValidation() {
        ...
        if (TestListener.getTest() != null) {
            TestListener.getTest().info("GET User endpoint: /users/2");
            TestListener.getTest().info("Response Status: " + response.getStatusCode());
    ...
    public void testUserNotFound_ErrorValidation() {
        ...
        if (TestListener.getTest() != null) {
            TestListener.getTest().info("GET User endpoint: /users/23");
